...
def VariantCalling(out, dict_param) :
    ...
    for file_ in os.listdir(out + "/01_ReadMapping/04.ReadRegrouping") :
        ...
        basename = basename.replace(".addRC.marked.sort.bam", '')
...
def Population(out, dict_param) :
    ...
    O_snphylo_param = open(f'{os.path.abspath(out)}/param/SNPhylo.txt', 'w')
    ...
    Population_input["SNPhylo"] = O_snphylo_param.name
    ...
    O_structure_param.write(out + "/03_Postprocessing/plink/" + dict_param["VariantCalling"]["VCF_prefix"] + ".bed\n")
    ...
    O_effectivesize_param = open(f'{os.path.abspath(out)}/param/effective_size.txt', 'w')
    for step in dict_param :
    ...
    O_admixtools_param = open(f'{os.path.abspath(out)}/param/admixtools.txt', 'w')
    for step in dict_param :
    ...
    O_admixtools_param.write(out + "/03_Postprocessing/plink/" + dict_param["VariantCalling"]["VCF_prefix"] + "\n")

    O_admixtools_param.close()
    Population_input["ADMIXTOOLS"] = O_admixtools_param.name

    O_LD_param = open(f'{os.path.abspath(out)}/param/LD.txt', 'w')
    for step in dict_param :
        if step == "LD" :
            for var in dict_param[step] :
    ...
    O_LD_param.close()
    Population_input['LD'] = O_LD_param.name
