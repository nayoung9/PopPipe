...
def Indexing(out, verbose) :
    ...
    sys.stderr.write("\n\nIndexing\n")
    sys.stderr.flush()
    if not os.path.isdir(out + "/02_VariantCalling/REF") :
        sub.call(f'mkdir {out}/02_VariantCalling/REF', shell=True)
    if not os.path.isdir(out + "/02_VariantCalling/logs/REF") :
        sub.call(f'mkdir {out}/02_VariantCalling/logs/REF', shell=True)
    ...
    value = sub.call(f'{line} 2> {log}', shell=True)
    if not value == 0 :
    ...
    if not DBSNP == "" :
        ...
        if STATE == 1 :
            line = "java -jar " + VariantTool + " -T IndexFeatureFile -F " + out + "/02_VariantCalling/REF/" + dbsnp_basename + " &> " + out + "/02_VariantCalling/logs/REF/dbsnp.index.log"
            if verbose == "1" :
                sys.stdout.write(line + "\n")
                sys.stdout.flush()

            value = os.system(line)
            if not value == 0 :
                sys.stdout.write("[ERROR] Check the log file : " + out + "/02_VariantCalling/logs/REF/dbsnp.index.log\n")
                sys.stdout.flush()
                sys.exit()
        elif STATE == 2 :
...
def LocalRealignment(out, th, memory, job, verbose) :
    ...
    for i,j in SAMPLE_DATA.items() :
        line = "java -Xmx" + memory + "g -jar " + PICARD + " ReorderSam " + localrealn_option_line + "INPUT=" + j + " OUTPUT=" + out + "/02_VariantCalling/01.LocalRealignment/" + i + ".reorder.addRG.marked.sort.bam REFERENCE=" + REFERENCE
    ...
    for i in SAMPLE_DATA.keys() :
        line = SAMTOOLS + " index " + out + "/02_VariantCalling/01.LocalRealignment/" + i + ".reorder.addRG.marked.sort.bam " + out + "/02_VariantCalling/01.LocalRealignment/" + i + ".reorder.addRG.marked.sort.bam.bai"
...
def IndelRealignment(out, th, memory, job, verbose) :
    ...
    for i,j in SAMPLE_DATA.items() :
        line = ""
        log = ""
        line = "java -Xmx" + memory + "g -jar " + VariantTool + " -T RealignerTargetCreator " + indelrealn_targetcreator_option_line + "-nt " + str(th) + " -R " + REFERENCE + " -I " + out + "/02_VariantCalling/01.LocalRealignment/" + i + ".reorder.addRG.marked.sort.bam -o " + out + "/02_VariantCalling/02.IndelRealignment/" + i + ".intervals -known " + DBSNP
    ...
    for i in error_sample :
        tmp_line = "java -Xmx" + memory +"g -jar " + VariantTool + " -T RealignerTargetCreator " + indelrealn_targetcreator_option_line + "--fix_misencoded_quality_scores -nt " + str(th) + " -R " + REFERENCE + " -I " + out + "/02_VariantCalling/01.LocalRealignment/" + i + ".reorder.addRG.marked.sort.bam -o " + out + "/02_VariantCalling/02.IndelRealignment/" + i + ".intervals -known " + DBSNP
        tmp_log = out + "/02_VariantCalling/logs/02.IndelRealignment/" + i + ".targetcreator2.log"
        new_cmd.append([tmp_line, tmp_log])
        if verbose == "1" :
            sys.stderr.write(tmp_line + " 2> " + log + "\n")
            sys.stderr.flush()
...
def BaseRecalibration(out, th, memory, job, verbose) :
    ...
    if STATE == 1 :
        ...
        for i, j in SAMPLE_DATA.items() :
            line = ""
            log = ""
            line = VariantTool + " BaseRecalibrator " + baserecal_option_line + "-I " + j + " -R " + REFERENCE + " --known-sites " + DBSNP + " -O " + out + "/02_VariantCalling/01.BaseRecalibration/" + i + ".table"
...
def GatkVariantcalling(out, th, memory, job, verbose) :
    ...
    if STATE == 1 :
        ...
        if DBSNP == "" :
            ...
            log = out + "/02_VariantCalling/logs/VariantCalling/" + VCF_prefix + ".genotyping.log"
        else :
        ...
        for i,j in sorted(SAMPLE_DATA.items()) :
            line, log = "", ""
            line = VariantTool + " --java-options \"-Xmx" + str(memory) + "g\" HaplotypeCaller " + variantcalling_option_line + "-R " + REFERENCE + " -I " + out + "/02_VariantCalling/01.BaseRecalibration/" + i + ".recal.addRG.marked.sort.bam -O " + out + "/02_VariantCalling/VariantCalling/" + i + ".variant.g.vcf.gz -ERC GVCF"
            log = out + "/02_VariantCalling/logs/VariantCalling/" + i + ".variant.log"
...
def SamtoolsVariantCalling(out, job, verbose) :
    global STATE
    global VCF_prefix
    global SAMTOOLS
    global BCFTOOLS
    ...
    CMD.clear()
    line = "(" + BCFTOOLS + " call -c -v --output-type b " + out + "/02_VariantCalling/VariantCalling/" + VCF_prefix + ".raw.bcf | " + BCFTOOLS + " view -m 2 --output-type b > " + out + "/02_VariantCalling/VariantCalling/" + VCF_prefix + ".variant.bcf)"
    ...
    sys.stderr.flush()
    CMD.clear()
    line = BCFTOOLS + " view " + out + "/02_VariantCalling/VariantCalling/" + VCF_prefix + ".variant.bcf | vcfutils.pl varFilter - > " + out + "/02_VariantCalling/VariantCalling/" + VCF_prefix + ".variant.flt.vcf"
...
def start(out, th, memory, job, verbose) :
    ...
    Indexing(os.path.abspath(out), verbose)
    ...
    if STATE == 1 :
        ...
        SamtoolsVariantCalling(os.path.abspath(out), job, verbose)
        sys.stderr.write("Success the Variant Calling\n")
        sys.stderr.flush()
    sys.stderr.write("Finish the Variant Calling\n")
    sys.stderr.flush()
...
def main() :
    ...
    if not os.path.isdir(args.out_dir + "/02_VariantCalling") :
        sub.call(f'mkdir {args.out_dir}/02_VariantCalling', shell=True)
    if not os.path.isdir(args.out_dir + "/02_VariantCalling/logs") :
        sub.call(f'mkdir {args.out_dir}/02_VariantCalling/logs', shell=True)
...
def main_pipe(args, dict_param, index) :
    ...
    if not os.path.isdir(args.out + "/02_VariantCalling") :
        sub.call(f'mkdir {args.out}/02_VariantCalling', shell=True)
    if not os.path.isdir(args.out + "/02_VariantCalling/logs") :
        sub.call(f'mkdir {args.out}/02_VariantCalling/logs', shell=True)
    ...
    sub.call(f'mkdir {args.out}/02_VariantCalling/VariantCalling/FINAL', shell=True)
