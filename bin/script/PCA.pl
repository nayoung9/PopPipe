...
`$gcta_cmd --bfile $plink_prefix --autosome-num $chr_num --autosome --make-grm --out $outdir/GRM/$prefix`;

### PCA
`mkdir -p $outdir/PCA`;
print "$gcta_cmd --grm $outdir/GRM/$prefix --pca $pca_num --out $outdir/PCA/$prefix\n";
`$gcta_cmd --grm $outdir/GRM/$prefix --pca $pca_num --out $outdir/PCA/$prefix`;
...
`Rscript $visPCA_R "$pca_title" $results_PCnum $outdir/PCA/$prefix.eigenval  $outdir/PCs.info $obj_variance  $outdir`;
