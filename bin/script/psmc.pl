...
#
#  Parameter 	
#  $./psmc.pl -t [threads] -p [parameter file] -o [out directory]
#
#  Input
#	Parameter file
#
...
foreach my $sample (keys %hs_multi){
	...
	`$samtools_cmd merge $outdir/$sample/$sample.merged.bam $bam_list`;
	$pm->finish;
}
...
foreach my $sample (keys %hs_multi){
	...
	`$samtools_cmd mpileup -C$sam_c -uf $ref_fa $outdir/$sample/$sample.merged.bam | $bcftools_cmd view -c - | $vcfutils_cmd vcf2fq -d $vcf_d | gzip > $outdir/$sample/$sample.diploid.fq.gz`;
	$pm->finish;
}
$pm->wait_all_children;
...
foreach my $sample (keys %hs_multi){
	...
	`$psmc_dir/utils/fq2psmcfa -q$q $outdir/$sample/$sample.diploid.fq.gz > $outdir/$sample/$sample.diploid.psmcfa`;
	$pm->finish;
}
$pm->wait_all_children;
...
foreach my $sample (keys %hs_multi){
	...
	`$psmc_dir/psmc -N$N -t$t -r$r -p $p -o $outdir/$sample/$sample.diploid.psmc $outdir/$sample/$sample.diploid.psmcfa`;
	$pm->finish;
}
$pm->wait_all_children;
...
`$psmc_dir/utils/psmc_plot.pl -p -M $legend_order $outdir/psmc_plot $psmc_files`;
